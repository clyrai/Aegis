...
# Aegis Retail (GDPR) Federated DP Training Script
# Focuses on strong privacy defaults and audit outputs.

command -v curl >/dev/null 2>&1 || { echo "curl is required"; exit 1; }
command -v jq >/dev/null 2>&1 || { echo "jq is required"; exit 1; }

BASE_URL=${BASE_URL:-http://localhost:8000}
SESSION_ID=${SESSION_ID:-gdpr_run_$(date +%s)}
ROUNDS=${ROUNDS:-6}
STRATEGY=${STRATEGY:-trimmed_mean}

# GDPR-leaning DP defaults (stricter)
CLIP=${CLIP:-0.8}
NOISE=${NOISE:-1.5}
...
# No-op to avoid unbound array errors if empty
declare -a CURLOPTS=()
...
for i in $(seq 1 $((ROUNDS*20))); do
  ...
  log "status=$st rounds=${cr}/${tr} epsilon≈${ep}"
  [[ "$st" == "completed" ]] && break
  sleep 1
done
